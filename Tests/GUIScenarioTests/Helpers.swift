...
func openFreshChat(
  ...
  let candidates: [(String, XCUIElement)] = [
    ("conversation-list new menu", app.menuButtons["chats.newButton"]),
  ]
  ...
  repeat {
    for (label, button) in candidates {
      ...
      sawCandidate = true
      button.click()
      let newChatItem = app.menuItems["New Chat"]
      guard newChatItem.waitForExistence(timeout: 2) else {
        NSLog("openFreshChat: %@ opened without a New Chat menu item", label)
        continue
      }
      newChatItem.click()
