...
/// The new-conversation menu is always the first list row; per-row delete
...
struct ChatListView: View {
  ...
  /// Fixed first row for every new-conversation mode. The visible control stays
  /// icon-only because the plus is the familiar creation symbol; menu item
  /// labels name the concrete mode after the user opens it.
  private var newChatMenu: some View {
    Menu {
      Button {
        beginChatDraft(profileID: ProfileStore.defaultProfileID)
      } label: {
        Label("New Chat", systemImage: "text.bubble")
      }
      Button {
        beginChatDraft(profileID: ProfileStore.nextTokenArenaProfileID)
      } label: {
        Label("Next Token Arena", systemImage: "gamecontroller")
      }
      Button {
        beginChatDraft(profileID: ProfileStore.probabilityLensProfileID)
      } label: {
      ...
      HStack {
        Image(systemName: "plus")
          .sidebarIcon()
          // A macOS borderless `Menu` re-tints a monochrome template symbol
          // with `NSButton`'s dark control color. Palette rendering preserves
          // the explicit light glyph used by this dark sidebar instead.
  ...
  private func beginChatDraft(profileID: String) {
    // Preserve the selected concrete model while the menu's chosen item owns
    // the new chat's profile/mode. With no current selection the profile's
    // default model remains unpinned.
    let source = selectedItemID.flatMap { id in chats.first { $0.id == id } }
    windowState.beginChatDraft(profileID: profileID, modelID: source?.modelID)
  }
