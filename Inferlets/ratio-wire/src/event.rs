...
pub struct GenResult {
    ...
    /// KV reuse diagnostics. `boundary_found` alone is not a hit: eviction
    /// SUSPENDS rather than deletes, so an evicted boundary opens successfully
    /// and replays the whole prefix. `reused_tokens` is NAME-level and cannot
    /// tell those apart on its own — read it against `replayed_pages`.
    #[serde(default)]
    pub boundary_found: bool,
    #[serde(default)]
    pub reused_tokens: u32,
    /// Committed pages the engine reported as already resident. `None` when the
    /// engine reported nothing (cold start, or an engine without the API).
    #[serde(default, skip_serializing_if = "Option::is_none")]
