...
/// KV reuse diagnostics, shared by every terminal result that crosses the
/// guest → gateway boundary: [`GenResult`], `tot_core::tree::TreeResult` and
/// `bestofn_core::round::RoundResult`.
///
/// One definition, three carriers, because the alternative was three copies
/// that could drift silently. `TreeResult` and `RoundResult` cannot be imported
/// into the gateway — both pull in the wasm-only `inferlet` SDK — so the
/// gateway used to read these by string with a `-1`/`false` fallback, and a
/// rename on the guest side compiled clean on both sides. It now decodes THIS
/// type instead, which makes the field names a compiler-checked contract.
///
/// Always `#[serde(flatten)]`ed into its carrier: the fields sit at the top
/// level of the JSON exactly as they did when each struct declared its own.
/// Nesting them would be a breaking wire change.
///
/// `boundary_found` alone is not a hit. Eviction SUSPENDS rather than deletes,
/// so an evicted boundary opens successfully and replays the whole prefix.
/// `reused_tokens` is NAME-level and cannot tell those apart on its own — read
/// it against `replayed_pages`.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct KvDiagnostics {
    #[serde(default)]
    pub boundary_found: bool,
    #[serde(default)]
    pub reused_tokens: u32,
    /// Committed pages the engine reported as already resident. `None` when the
    /// engine reported nothing (cold start, or an engine without the API) — so
    /// "no data" never reads as a confirmed zero.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub resident_pages: Option<u32>,
    /// Committed pages the engine REGENERATED by replay. Non-zero falsifies the
    /// reuse claim above: the name resolved, the KV did not survive.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub replayed_pages: Option<u32>,
    /// Recurrent state was replayed — independent of `replayed_pages`, and the
...
pub struct GenResult {
    ...
    /// KV reuse diagnostics — see [`KvDiagnostics`]. Flattened, so these stay
    /// at the top level of the JSON where they have always been.
    #[serde(flatten)]
